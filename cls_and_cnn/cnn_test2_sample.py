...
cnn_test2 = '/Users/wywy/Desktop/cnn_test2'
cnn_test2_filename = './cnn_test2.tfrecords'  # 输出文件地址


def saver_lables(lables_path,train_filename):
    writer = tf.python_io.TFRecordWriter(train_filename)
    for i in os.listdir(cnn_test2):
        if i=='.DS_Store':
            pass
        else:
            ...
            if sample=='A':
                ...
                pass
            img=Image.open(cnn_test2+'/'+i)
            xx=iming.imread(cnn_test2+'/'+i)
            image = img.tobytes()
...
def cnn_test2_shuffle_batch(train_file_path, image_size,batch_size, capacity=7000, num_threads=3):
...
if __name__=='__main__':
    saver_lables(cnn_test2,cnn_test2_filename)
    # read_data_for_file(train_filename,100,[16,88,3])
    a=cnn_test2_shuffle_batch(cnn_test2_filename,[18,88,3],100)
